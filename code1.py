...
try:
    ...
    cursor = conn.cursor()
    sql = """insert into books (book_name,author_name,book_path) values('%s','%s','%s')
    """%(book_name,book_author,book_path)
    result = cursor.execute(sql)
    print(result)
    conn.commit()
    sql01 = "select id from books where book_path = '%s'" % book_path
    cursor.execute(sql01)
    book_id = "B" + str(cursor.fetchall()[0][0])
    print(book_id)
    sql02 = """create table %s(section_id int(8) primary key auto_increment,book_section varchar(128),section_path varchar(128))default charset=utf8;
    """%book_id
...
try:
    ...
    for i in value:
        ...
        if len(path) != 0:
            ...
            sql03 = "insert into %s(book_section,section_path) values('%s','%s')" % (book_id,path[0],os.path.abspath(new_path))
